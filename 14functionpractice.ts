...
function greet(person:string, messgae:string){
...
function Calculate_area(length:number,width:number,unit:string="meters"){
...
function Greet(name:string,language:string='English'){
    if(language==="Spanish"){
...
function getTotalPrice(price:number,taxrate:number=0.08){
...
function calculateTotalprice(price:number,discount?:number){
...
function createUsers(name:string,email:string, role?:string){
    if (role===undefined){
        return `${name} ${email}`
    }
    else{
...
function getPersonInfo(name:string, age:number,occupation?:string){
...
function getUserName(firstname:string, lastName:string, title?:string){
    if(title!==undefined){
