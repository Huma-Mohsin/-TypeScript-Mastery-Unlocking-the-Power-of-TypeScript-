...
let p1:person=person.center;
console.log(p1);//Governor House karachi

let p2:person=person["NumberOfDaya"];
console.log(p2);//3

let p3:person=person.id;
console.log(p3);//0
...
let s1:status=status["rejected"];
console.log(s1);

// let s2:status=status[404]
// console.log(s2);//rejected


/*const enum is used  to make constant values. we cannot access value in const enum by defining index */

const enum stationary{
    red,
    green=5,
    blue="blues",
}

let st1:stationary=stationary["red"];
console.log(st1);//0

// let st2:stationary=stationary[5];//A const enum member can only be accessed using a string literal.

let st2:stationary=stationary.blue;
