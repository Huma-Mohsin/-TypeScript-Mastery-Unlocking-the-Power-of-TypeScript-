...
function printPersonDetails(param) {
...
Object.entries(john).forEach(([key, value]) => console.log(`${key}:${john[key]}`));
//or
for (const key in john) {
    if (typeof john[key] === "object") {
        console.log(`${key}:${JSON.stringify(john[key])}`);
    }
    else {
...
for (const key in carWithExtraFeatures) {
    console.log(`${key} : ${carWithExtraFeatures[key]}`);
}
// 7. Deletes the year property from the merged object.
delete carWithExtraFeatures.year; //data type problem so here we i type assertion .
console.log(carWithExtraFeatures);
...
function hasLeatherSeats(param) {
...
function mergeVehicles(vehicle1, vehicle2) {
